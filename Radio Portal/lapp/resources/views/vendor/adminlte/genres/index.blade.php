...
@section('content_header', __('admin.genres'))
...
<div class="row">
    <div class="col-12">
        <a href="{{ asset(env('ADMIN_URL').'/genres/create') }}" class="btn button-green mb-3"><i class="fas fa-plus-square"></i>
            @lang('admin.create_genre')</a>
 <a href="{{ asset(env('ADMIN_URL').'/genres/sort') }}" class="btn button-dark ml-2 mb-3"><i class="fas fa-arrows-alt"></i>
            @lang('admin.sort_genres')</a>    
             <a href="{{ asset(env('ADMIN_URL').'/genres/sort_home') }}" class="btn button-purple ml-2 mb-3"><i class="fas fa-arrows-alt"></i>
            @lang('admin.sort_genres_home')</a>   
        <div class="card">
            <div class="table-responsive">
                <table class="table table-hover text-nowrap m-0" id="table" data-delete-prompt-title="@lang('admin.oops')" data-delete-prompt-body="@lang('admin.delete_prompt')" data-yes="@lang('admin.yes')" data-cancel="@lang('admin.cancel')">
                    ...
                    <tbody>
                        ...
                        <tr id="{{$row->id}}">
                            <td>{{$row->id}}</td>
                            <td><a href="{{ asset($settings['genre_base']) }}/{{ $row->slug }}" class="text-dark" target="_blank">{{$row->title}}</a></td>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu mr-3">
                                        ...
                                        <form id="delete_from_{{$row->id}}" method="POST" action="{{action('App\Http\Controllers\GenreController@destroy', $row['id'])}}">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
